...
def removeDisplayViews(self, out, dict):
    # remove the views for the defined content type(s)
    tool = getToolByName(self, 'portal_types')
    for pt, views in dict.items():
        FTI = getattr(tool, pt)
        nviews = [v for v in FTI.view_methods if v not in views]
        FTI.view_methods = tuple(nviews)
        print >> out, "Removed display views from %s in portal_types." % pt
